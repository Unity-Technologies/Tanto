...
const reviewerItem = (props : MissingReviewerListType, reviewer : UserType) => {
  let clickFunction = () => props.addReviewer(props.id, reviewer)
...
const MissingReviewers = (props: MissingReviewerListType) => {
  let missing = props.missingReviewers.filter(r => r.reviewers != null && r.reviewers.length > 0)
...
const UnclaimedAreas = (props: MissingReviewerListType) => {
  let unclaimed = props.missingReviewers.filter(r => r.reviewers == null || r.reviewers.length == 0)
  let reviewPage = 'https://confluence.hq.unity3d.com/display/DEV/Code+Reviews'
