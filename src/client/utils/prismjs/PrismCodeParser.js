...
export const unifiedDiff = (diff: string, type: string) => {
  ...
  for (let i = 0, arrayLength = codeLines.length; i < arrayLength; i += 1) {
    line = codeLines[i]
...
export const sideBySideDiff = (diff: string, type: string) => {
  const html = Prism.highlight(diff, Prism.languages[type] || Prism.languages.clike)
  const codeLines = html.split('\n')
  const addedOperator = '<span class="token operator" >+</span>'
  const removedOperator = '<span class="token operator" >-</span>'
  ...
  const result = []
  for (let k = 0, length = oldLines.length; k < length; k += 1) {
